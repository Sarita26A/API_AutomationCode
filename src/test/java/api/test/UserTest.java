...
public class UserTest {
    ...
    public void testGetReq()
    {
        ...
        Response response=UserEndPoints.readUser(this.payloads.getUsername());
        response.then().log().all();
        Assert.assertEquals(response.getStatusCode(),200);
    ...
    public void testUpdateUserByName()
    {
        ...
        payloads.setLastName(faker.name().lastName());
        payloads.setEmail(faker.internet().emailAddress());
        Response response=UserEndPoints.updateUser(this.payloads.getUsername(),payloads);
        response.then().log().all();
        Assert.assertEquals(response.getStatusCode(),200) ;
        //Checking Data afetr updation
        Response responseAfterUpdate= UserEndPoints.readUser(this.payloads.getUsername());
